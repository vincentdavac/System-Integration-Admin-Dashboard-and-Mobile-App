...
import LogoIcon from '/icons/new_icon.svg';
import DarkModeSwitcher from './DarkModeSwitcher';
import { AlertsContainerRef } from '../Alert/AlertsContainer';

interface HeaderProps {
  sidebarOpen: boolean; // no need for string | undefined
  setSidebarOpen: (open: boolean) => void;
  alertsRef: React.RefObject<AlertsContainerRef>;
}

const Header = ({ sidebarOpen, setSidebarOpen, alertsRef }: HeaderProps) => {
  return (
    <header className="sticky top-0 z-10  flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
      <div className="flex flex-grow items-center justify-end px-4 py-4 shadow-2 md:px-6 2xl:px-11">
        ...
        <div className="flex items-center gap-3 2xsm:gap-7">
          ...
          <DropdownUser alertsRef={alertsRef} />
        </div>
      </div>
    </header>
  );
